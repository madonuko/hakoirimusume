(function() {
  ...
  console.log('[macro-hover] script loaded');
  ...
  function portalAllPopups() {
    console.log('[macro-hover] portalAllPopups called');
    var popupDataScript = document.getElementById('macro-popup-data');
    if (!popupDataScript) {
      console.log('[macro-hover] popup data script not found');
      return;
    }
    ...
    try {
      ...
      console.log('[macro-hover] parsed popup data, count:', Object.keys(popupData).length);
    } catch (e) {
    ...
    Object.keys(popupData).forEach(function(popupId) {
      console.log('[macro-hover] processing popup:', popupId);
      if (document.querySelector('[data-original-popup-id="' + popupId + '"]')) {
        console.log('[macro-hover] popup already exists:', popupId);
        return;
      }
      ...
      console.log('[macro-hover] added popup:', popupId);
    });
  }

  function initMacroPopups() {
    console.log('[macro-hover] initMacroPopups called');
    portalAllPopups();

    console.log('[macro-hover] looking for wrappers');
    document.querySelectorAll('.macro-hover-wrapper').forEach(function(wrapper) {
      ...
      console.log('[macro-hover] found wrapper:', popupId);
      if (!popupId || !trigger) return;

      var portalledPopup = document.querySelector('[data-original-popup-id="' + popupId + '"]');
      console.log('[macro-hover] found portalled popup:', !!portalledPopup);
      ...
      function showPopup() {
        ...
        portalledPopup.setAttribute('aria-hidden', 'false');
      ...
      function hidePopup() {
        hideTimeout = setTimeout(function() {
          ...
          portalledPopup.setAttribute('aria-hidden', 'true');
  ...
  function setupMacroPopups() {
    console.log('[macro-hover] document.readyState:', document.readyState);
    console.log('[macro-hover] macro-popup-data exists:', !!document.getElementById('macro-popup-data'));
    console.log('[macro-hover] macro-hover-wrapper count:', document.querySelectorAll('.macro-hover-wrapper').length);
